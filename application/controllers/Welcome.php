<?php
	...
	class Welcome extends CI_Controller
	{
		...
		{
			$data = [
				'Number' => $this->security->xss_clean($this->input->post('username')),
			...
			if (!empty($result)) {
				if (($result->SecurityLevel === NULL) || ($result->SecurityLevel === 0)) {
					...
					$data = [
						'main_content' => 'Login successful',
						'stocktakestatus' => $this->Stocktake->stocktakestatus()
					];
		...
		public function dashboard()
		{
			...
			$data = [
				'title' => 'Dashboard',
				'stocktakestatus' => $this->Stocktake->stocktakestatus(),
				'config' => $this->Stocktake->storeconfig(),
				'progress' => $this->Stocktake->sheetprogress(),
				'total' => $this->Stocktake->syncstocksheetsval()
			];
		...
		public function users()
		{
			...
			$data = [
				'title' => 'Users',
				'users' => $this->Stocktake->system_users(),
				'userlevels' => $this->Stocktake->userlevels(),
				'branches' => $this->Stocktake->branches(),
				'config' => $this->Stocktake->storeconfig()
			];
		...
		public function items()
		{
			...
			$data = [
				'title' => 'Items',
				'departments' => $this->Stocktake->departments(),
				'categories' => $this->Stocktake->category(),
				'subcategories' => $this->Stocktake->subcategory(),
		...
		public function suppliers()
		{
			...
			$data = [
				'title' => 'Suppliers',
				'suppliers' => $this->Stocktake->suppliers(),
				'config' => $this->Stocktake->storeconfig()
			];
		...
		public function customers()
		{
			...
			$data = [
				'title' => 'Customers',
				'customers' => $this->Stocktake->customers(),
				'config' => $this->Stocktake->storeconfig()
			];
		...
		public function transactions()
		{
			...
			$data = [
				...
				'stocktakestatus' => $this->Stocktake->stocktakestatus(),
				'products' => $this->Stocktake->transactions(),
				'config' => $this->Stocktake->storeconfig()
			];
		...
		public function fstocks()
		{
			...
			$data = [
				'title' => 'Stocks Freeze',
				'config' => $this->Stocktake->storeconfig(),
				'freeze' => $this->Stocktake->stocktakestatus(),
				'stocktakestatus' => $this->Stocktake->stocktakestatus(),
				'stocktakeprogress' => $this->Stocktake->stocktakeprogress(),
		...
		public function stocksposting()
		{
			...
			$data = [
				...
				'config' => $this->Stocktake->storeconfig(),
				'stocktakestatus' => $this->Stocktake->stocktakestatus(),
				'pendingsyncrecords' => $this->Stocktake->pendingsyncrecords(),
				'pendings' => $this->Stocktake->pending_tempsheets(),
				'tempsheets_status' => $this->Stocktake->tempsheets_status()
			];
		...
		public function fsheets()
		{
			...
			$data = [
				'title' => 'Feed Sheets',
				'stocktakestatus' => $this->Stocktake->stocktakestatus(),
				'tsheets' => $this->Stocktake->tempsheets(),
				'shelf' => $this->Stocktake->shelf(),
				'reason' => $this->Stocktake->reasoncode(),
				'config' => $this->Stocktake->storeconfig()
			];
		...
		public function fetch_entries()
		{
			...
			$lookupCode = $this->input->post('LookupCode', true);
			$start = $this->input->post('start', true) ?: 0;
			$length = $this->input->post('length', true) ?: 10; // Match DataTables default
			$search = $this->input->post('search[value]', true);
			$order_column = $this->input->post('order[0][column]', true) ?: 0; // Default to column 0
			$order_dir = $this->input->post('order[0][dir]', true) ?: 'ASC';
			
			// Map DataTables column indices to model keys
			$columns = [
				0 => 'ID',
				1 => 'Username',  // Adjusted to match view column order
				2 => 'bin',       // Adjusted to match view column order
				3 => 'ItemLookupCode',
				4 => 'Alias',
				5 => 'Description',
				6 => 'Cost',
				...
				9 => 'costValue',
				10 => 'priceValue',
				11 => 'StocktakeID', // Not displayed but included in data
				12 => 'ItemID'      // Not displayed but included in data
			];
		...
		public function syncstocksheets()
		{
			...
			// Header title
			$data = [
				'title' => 'Synchronise sheets',
				'config' => $this->Stocktake->storeconfig(),
				'stocktakestatus' => $this->Stocktake->stocktakestatus(),
				'stocktakesheets' => $this->Stocktake->stocktakesheets(),
				'tempsheets_status' => $this->Stocktake->tempsheets_status(),
				'pendingsyncrecords' => $this->Stocktake->pendingsyncrecords(),
				'pendings' => $this->Stocktake->pending_tempsheets(),
				'shelf' => $this->Stocktake->shelf(),
		...
		public function specific_feed()
		{
			...
			$data = [
				...
				'stocktakesheets' => $this->Stocktake->stocktakesheets(),
				'total' => $this->Stocktake->syncstocksheetsval(),
				'psynchs' => $this->Stocktake->specific_feed($LookupCode),
				'shelf' => $this->Stocktake->shelf(),
				'config' => $this->Stocktake->storeconfig()
			];
		...
		public function history()
		{
			...
			$data = [
				...
				'history' => $this->Stocktake->history(),
				'config' => $this->Stocktake->storeconfig()
			];
		...
		public function stocktakedetails($id)
		{
			...
			$data = [
				...
				'info' => $this->Stocktake->get_record($id),
				'config' => $this->Stocktake->storeconfig()
			];
		...
		public function historysearch()
		{
			...
			$data = [
				...
				'subcategories' => $this->Stocktake->subcategory(),
				'users' => $this->Stocktake->users(),
				'freeze' => $this->Stocktake->historysearch($user, $startdate, $enddate)
			];
