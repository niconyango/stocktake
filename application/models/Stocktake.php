...
// stock holding report in excel.
		...
		public function updatedetail()
		{
			...
			if ($id > 0) {
				// Update stocksheets table
				$this->db->where('ID', $id);
				if (!$this->db->update('stocksheets', $data)) {
					//log_message('error', 'Failed to update stocksheets for ID: ' . $id);
					return false;
				}
				
				// Update tempsheets table
				$update_data = [
					'Quantity' => $data['Quantity']
				];
				
				$this->db->where('ItemID', $this->input->post('item_id'));
				$this->db->where('ItemLookupCode', $data['ItemLookupCode']);
				$this->db->where('Quantity', (float)$this->input->post('curquantity'));
				$this->db->where('CashierName', $this->input->post('user', true));
				$this->db->where('StocktakeID', (int)$this->input->post('StockTakeID'));
				
				if (!$this->db->update('tempsheets', $update_data)) {
					//log_message('error', 'Failed to update tempsheets for ItemLookupCode: ' . $data['ItemLookupCode']);
					return false;
				}
...
//
		...
		private function _syncstocksheets($search, $order_by, $order_dir, $LookupCode)
		{
			$this->db->select("s.ID, s.StocktakeID, s.Description, s.bin, s.Username, s.Quantity, a.`Alias`, s.ItemID, s.`ItemLookupCode`, i.`Cost`, (s.Quantity * i.Cost) AS costValue, i.`Price`, (i.`Price` * s.`Quantity`) AS priceValue");
			$this->db->from('stocksheets s');
			$this->db->join('item i', 'i.ID = s.ItemID');
			$this->db->join('alias a', 'a.ItemID = s.ItemID', 'LEFT');
			$this->db->where('s.Status', 0);
			...
			if (!empty($LookupCode)) {
				$this->db->group_start()
					->where('s.ItemLookupCode', $LookupCode)
					->or_where('a.Alias', $LookupCode)
					->or_where('i.ItemLookupCode', $LookupCode)
					->group_end();
			}
			...
			$sortable_columns = [
				...
				'ItemID' => 's.ItemID',
				'ItemLookupCode' => 's.ItemLookupCode',
				'Alias' => 'a.Alias',
				...
				'Cost' => 'i.Cost',
				'Price' => 'i.Price',
				'costValue' => '(s.Quantity * i.Cost)',
				'priceValue' => '(i.Price * s.Quantity)'
			];
			
			$order_by_column = array_key_exists($order_by, $sortable_columns) ? $sortable_columns[$order_by] : 's.ID';
		...
		{
			$this->db->where('Status', 0);
			return $this->db->count_all_results('stocksheets');
		}
